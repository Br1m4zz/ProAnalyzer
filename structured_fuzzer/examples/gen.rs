...
use structured_fuzzer::mutator::{Mutator,MutatorSnapshotState};
...
//这个小程序用于基于已有的spec，生成指定LENGTH，变异NUMBER次数的测试用例
fn main() {
    ...
    mutator.generate(length, &MutatorSnapshotState::none(), &mut storage, &dist);
...
//         }
//         let strategy  = mutator.mutate(graph, graph.node_len(&mutator.spec) &CustomDict::new(),  &MutatorSnapshotState::none(), &queue, &mut storage, &dist);
