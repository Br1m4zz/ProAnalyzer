...
mod graph_spec {
    ...
    use super::*;
    use super::mutator::MutatorSnapshotState;
    use std::rc::Rc;
    use std::sync::Arc;
    ...
    fn builder(dict: Vec<Vec<u8>>) -> (GraphBuilder, VecGraph) {
        let spec = build_spec();
        let dist = crate::random::distributions::Distributions::new(dict);
        let mut gb = GraphBuilder::new(Rc::new(spec));
        let mut storage = VecGraph::new(vec![], vec![]);
        gb.start(&mut storage, &MutatorSnapshotState::none());
    ...
    fn build_redundant_graph(gb: &mut GraphBuilder, st: &mut VecGraph, dist: &Distributions) -> VecGraph {
        ...
        gb.start(st,&MutatorSnapshotState::none());
...
//     }
    ...
    fn test_fuzz_splice_random_limited_data() {
        ...
        mutator.generate(100, &mutator::MutatorSnapshotState::none(), &mut storage, &dist);
        let graph = mutator.dump_graph(&storage);
        let cnt = 1000;
        for _i in 0..cnt {
            mutator.splice_random(&graph, graph.node_len(&mutator.spec), &mutator::MutatorSnapshotState::none(), &custom_dict::CustomDict::new(), &mut storage, &dist);
        }
    }
}
